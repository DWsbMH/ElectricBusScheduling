...
# A,1,1 után A,1,2
s.t. SubsequentTask1 {(p,b,n) in Tasks:n!=1}:
	stime[p,b,n] >= ctime [p,b,n-1];
#A,1,vége után A,2,eleje
s.t. SubsequentTask2 {(p,b,n) in Tasks,(p,b,n2) in Tasks :b!=1}:
	stime[p,b,n] >= ctime [p,b-1,n2];
...
s.t. Makespanconstraint{(p,b,n) in Tasks}:
